...
If you run planning agents, their scratch (`task_plan.md`, `.planning/`, `.codex/`, `.claude/`, `.omx/`, …) lives *inside* the worktree — so it dies when you `wtrm`. The helpers carry it for you:
...
The archive lives outside any repo, so it's never committed and never clutters your notes app — browse it in your editor any time. Tune extra root-level files with `WT_PLAN_FILES`, where the archive lives with `WT_PLAN_ARCHIVE`, or skip it for one command with `WT_NO_PLAN=1`. The `.planning`, `.codex`, `.claude`, `.omc`, `.omx`, `.agents`, and `.cursor` directories are always protected unless `WT_NO_PLAN=1` is set.
...
| `WT_PLAN_FILES` | `task_plan.md findings.md progress.md .planning` | Space-separated files/dirs carried across a worktree's lifecycle. Agent-local state dirs are always included as safety items unless `WT_NO_PLAN=1` is set. |
