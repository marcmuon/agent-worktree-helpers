...
If you run planning agents, their scratch (`task_plan.md`, `.planning/`, `.omx-briefs/`, …) lives *inside* the worktree — so it dies when you `wtrm`. The helpers carry it for you:
...
The archive lives outside any repo, so it's never committed and never clutters your notes app — browse it in your editor any time. `task_plan.md`, `findings.md`, `progress.md`, `.planning/`, and `.omx-briefs/` are **always** carried; `WT_PLAN_FILES` adds extra root-level files/dirs on top. Choose where the archive lives with `WT_PLAN_ARCHIVE`, or skip archiving for one command with `WT_NO_PLAN=1`. Agent config/state directories (`.claude`, `.codex`, `.cursor`, `.agents`, `.omc`, `.omx`) are deliberately *not* archived — they are typically committed to the repo or hold caches and logs, not plans.
...
| `WT_PLAN_FILES` | _(unset)_ | Space-separated *extra* files/dirs to carry across a worktree's lifecycle, in addition to the always-carried planning set (`task_plan.md findings.md progress.md .planning .omx-briefs`). |
